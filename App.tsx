...
const App: React.FC = () => {
  ...
  const handleHostScan = useCallback(async (orderId: string, clientName?: string): Promise<ResolvedRouteInfo> => {
    ...
    // Wait for result directly (now that handleSearch returns a Promise<Result>)
    const result = await handleSearch(orderId, { isRemoteScan: true, clientId: clientName || 'Client' });

    if (result) {
      return result;
    } else {
      // Fallback if no result returned (e.g. error or exception handled internally without return)
      return {
        orderId: orderId.toUpperCase(),
        address: '',
        date: new Date().toLocaleDateString(),
        resolvedAt: new Date().toISOString(),
        exceptionReason: 'Scan processing failed or returned no result',
      };
    }
  }, [handleSearch]);
